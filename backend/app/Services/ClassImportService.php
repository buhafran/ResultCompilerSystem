...
final class ClassImportService
{
            ...
            DB::transaction(function () use ($school, $handle, $header, $maximumRows, &$created, &$updated, &$failed, &$errors, &$rowNumber): void {
                while (($row = fgetcsv($handle, null, ',', '"', '')) !== false) {
                    ...
                    if ($rowNumber > $maximumRows + 1) {
                        throw new RuntimeException("A maximum of {$maximumRows} class rows can be imported at once.");
                    }
                    ...
                    try {
                        ...
                        if ($class) {
                            ...
                            SchoolClass::create(['school_id' => $school->id, ...$payload]);
                            $created++;
                        }
    ...
    /** @return array{name:string,level:?string,arm:?string,is_active:bool} */
    private function validateRow(array $data, int $rowNumber): array
    {
        $name = trim((string) ($data['name'] ?? ''));

        if ($name === '') {
            throw new RuntimeException("Row {$rowNumber}: class name is required.");
        }

        if (mb_strlen($name) > 255) {
            throw new RuntimeException("Row {$rowNumber}: class name cannot exceed 255 characters.");
        }

        return [
            'name' => $name,
            'level' => $this->limitedNullableString($data['level'] ?? null, 255, $rowNumber, 'level'),
            'arm' => $this->limitedNullableString($data['arm'] ?? null, 255, $rowNumber, 'arm'),
    ...
    private function normaliseHeader(string $header): string
    {
        ...
        return match ($header) {
            'class', 'class_name', 'school_class', 'classname' => 'name',
            'class_level' => 'level',
            'class_arm', 'section' => 'arm',
