...
class TeacherAssignmentResource extends TenantManagedResource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Select::make('user_id')->label('Teacher')->options(fn (): array => Filament::getTenant()->users()
                ->wherePivot('is_active', true)->wherePivot('role', MembershipRole::Teacher->value)
                ->orderBy('users.name')->pluck('users.name', 'users.id')->all())->searchable()->preload()->required(),
            Select::make('academic_term_id')->options(fn (): array => AcademicTerm::query()->with('academicSession')
                ->where('school_id', Filament::getTenant()->id)->orderByDesc('starts_on')->get()
                ->mapWithKeys(fn (AcademicTerm $term): array => [$term->id => $term->academicSession->name.' - '.$term->name])->all())
